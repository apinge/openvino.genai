...
OPENVINO_GENAI_EXPORTS PerfMetricsHandle* AddPerfMetrics(const PerfMetricsHandle* left,
                                                         const PerfMetricsHandle* right) {
    if (left && right) {
        PerfMetricsHandle* result = new PerfMetricsHandle;
        result->object = std::make_shared<ov::genai::PerfMetrics>(*(left->object) + *(right->object));
        return result;
    }
    return NULL;
    }
